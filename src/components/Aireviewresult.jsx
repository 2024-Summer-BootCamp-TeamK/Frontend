...
const Aireviewresult = ({ contractDataMain, contractDataToxin }) => {
  ...
  useEffect(() => {
    if (
      contractDataMain &&
      contractDataMain.contract &&
      contractDataToxin &&
      contractDataToxin.contract
    ) {
      const plainText = modifyHtml(contractDataMain.contract); // 두 데이터의 텍스트가 동일하다고 가정

      const highlightedTextMain = highlightPlainText(
        plainText,
        contractDataMain.articles,
        "main"
      );
      const highlightedTextToxin = highlightPlainText(
        highlightedTextMain,
        contractDataToxin.articles,
        "toxin"
      );
